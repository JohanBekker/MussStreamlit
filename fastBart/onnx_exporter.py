...
from .onnx_models_structure import (
    BartEncoder,
    BartDecoderWithLMhead,
    BartDecoderWithLMheadInitial,
)
...
def create_bart_encoder_decoder(pretrained_version="facebook/bart-base"):
    ...
    model = BartForConditionalGeneration.from_pretrained(pretrained_version, use_auth_token=get_auth_token())
...
def turn_model_into_encoder_decoder(model):
    ...
    decoder_with_lm_head = BartDecoderWithLMhead(decoder, lm_head, final_logits_bias, model.config)
    simplified_encoder = BartEncoder(encoder)
    decoder_with_lm_head_init = BartDecoderWithLMheadInitial(decoder, lm_head, final_logits_bias, model.config)
...
def generate_onnx_representation(
    ...
    onnx_opset_version=15,  # no other opset versions are tested, change at your own risk
):
    ...
    if model is not None:
        ...
        (
            ...
            decoder_with_lm_head,
            decoder_with_lm_head_init,
        ) = create_bart_encoder_decoder(pretrained_version)
    ...
    attention_mask_dec = torch.ones(batch_size, enc_seq_length, dtype=torch.int64)
    enc_out = torch.ones(
    ...
    with torch.no_grad():
        ...
        if not Path(init_decoder_path).exists():
...
def quantize(models_name_or_path):
    ...
    for model in models_name_or_path:
        ...
        #optimize = True if "init" not in output_model_name else False
        if not Path(output_model_name).exists():
            quantize_dynamic(
